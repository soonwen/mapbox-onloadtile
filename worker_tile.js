...
class WorkerTile {
    ...
    parse(data: VectorTile, layerIndex: StyleLayerIndex, availableImages: Array<string>, actor: Actor, callback: WorkerTileCallback) {
        ...
        for (const sourceLayerId in layerFamilies) {
            ...
            if (sourceLayerId.indexOf('code')>=0){
               // debugger;
            }
            ...
            for (let index = 0; index < sourceLayer.length; index++) {
                ...
                if (sourceLayerId.indexOf('test_layer')>=0){
                    if (!feature.properties.hasOwnProperty('radius')){
                        feature.properties['radius']=Math.floor(Math.random() * 10); 
                        feature.properties['color'] = '#' + Math.floor(Math.random() * 16777215).toString(16);
                    }
                }
